...
public class Funciones {
    ...
    private static boolean isComentarioVariasLineas = false;
    ...
    public void limpiarCodigo(String linea) {
        ...
        for (int i = 0; i < linea.length(); i++) {
            c = linea.charAt(i);
            //Siguientes lineas de codigo usadas para eliminar los comentarios del codigo
            if (linea.charAt(i) == '/') {
                if (linea.charAt(i + 1) == '/') {
                    return;
                }
                if (linea.charAt(i + 1) == '*') {
                    isComentarioVariasLineas = true;
                }
            } else if (linea.charAt(i) == '*' && linea.charAt(i + 1) == '/') {
                isComentarioVariasLineas = false;
            }
            if (!isComentarioVariasLineas) {
                this.misToken(c);
            }
        }
    }
    ...
    public void marcar() {
        //Booleano usado para checar si es el primer ciclo encontrado
        ...
        while (it.hasNext()) {
            ...
            //Si la linea contiene un ciclo...
            if (linea.contains("if") || linea.contains("for") || linea.contains("while")) {
                //Agregamos el if a la linea, para evitar que se pierda mas adelante
                nuevoArreglo.add(linea + conc);
                //Aumentamos por 1 la cuenta de ciclos
                cuentaCiclo++;
            } else //Si encontramos que la linea contiene una llave { , que la cuenta de Ifs sea mayor a 0
            //Y que la cuenta de else sea menor a la de los ciclos...
            if (linea.contains("{") && cuentaCiclo > 0 && cuentaElse < cuentaCiclo) {
                ...
                nuevoArreglo.add(linea + conc);
                //Si no es ciclo anidado, sumamos 2 a la cuenta del numero ciclomatico, de otra forma sumamos 1
                if (primerCiclo) {
                    this.numeroCiclomatico += this.numeroCiclomatico + 2;
                    primerCiclo = !primerCiclo;
                } else {
                    this.numeroCiclomatico++;
                }
            ...
            if (linea.contains("}") && cuentaCiclo > 0 && !arreglo.get(pasos + 1).toString().contains("else")) {
    ...
    public int getNumeroCiclomatico() {
